# Reference: `Code.gs` (Apps Script source as Markdown)

This is a **verbatim mirror of `apps-script/Code.gs`**, provided in `.md` form so it
can be attached to a Claude Code agent (the `.gs` extension isn't an accepted
attachment type). It is the canonical Apps Script template referenced by both
`docs/META_CAPI_SOP_VSL.md` and `docs/META_CAPI_SOP_WEBINAR.md`.

**For the receiving agent:** copy the code block below into the client's Sheet
Apps Script editor as `Code.gs`, then customize the two funnel-specific things
per the relevant SOP:

1. The `EVENTS` config block (event names + which columns trigger them).
2. The Script Properties (`META_PIXEL_ID`, `META_CAPI_ACCESS_TOKEN`,
   `EVENT_SOURCE_URL_DEFAULT`) — set in the Apps Script project, NOT in code.

Everything else (SHA-256 hashing, `user_data` construction, retry/backoff,
`_Errors` logging, `setupTriggers`, `replayPendingEvents`, the `onSheetEdit`
dispatcher) stays identical. Reuse it; do not rewrite.

> If this file and `apps-script/Code.gs` ever diverge, `Code.gs` is the source
> of truth. Re-sync this mirror from it.
